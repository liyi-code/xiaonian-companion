 ...
 *
 * 它与 NpcAgent 挂在同一 GameObject（小念的 VRM 模型）上。
 * Python 端的 AutonomousExplorer 经 BridgeHub 下发 agent_command，由 NpcAgent 转发到这里执行：
 ...
 * 移动基于 Transform；若你的项目用 NavMesh / 物理角色控制器，把移动内部替换即可。
 */
...
public class AgentController : MonoBehaviour
{
    ...
    // 由 NpcAgent.ApplyAgentCommand 调用
    public void HandleCommand(string action, Vector3? target, string objectId)
    {
        ...
        switch (action)
        {
            case "move":
                if (target.HasValue)
                {
                    moveTarget = target.Value;
                    faceTarget = false;   // 移动时朝移动方向转身，取消“看”的抢转
                }
                break;
            case "look":
                if (target.HasValue) { lookAt = target.Value; faceTarget = true; }
                break;
            case "interact":
                DoInteract(objectId);
                break;
